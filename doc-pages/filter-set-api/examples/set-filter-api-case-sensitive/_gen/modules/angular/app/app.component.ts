...
import { ColDef, ColGroupDef, ColumnApi, FirstDataRenderedEvent, Grid, GridApi, GridOptions, GridReadyEvent, ICellRendererParams, IFiltersToolPanel, ISetFilter, SideBarDef } from '@ag-grid-community/core';
...
@Component({
    ...
    <div class="example-header">
        <div>
            Case Insensitive:
            <button (click)="setModel('insensitive')">API: setModel() - mismatching case</button>
            <button (click)="getModel('insensitive')">API: getModel()</button>
            <button (click)="setFilterValues('insensitive')">API: setFilterValues() - mismatching case </button>
            <button (click)="getValues('insensitive')">API: getValues()</button>
            <button (click)="reset('insensitive')">Reset</button>
        </div>
        <div style="padding-top: 10px;">
            Case Sensitive:
            <button (click)="setModel('sensitive')">API: setModel() - mismatching case</button>
            <button (click)="getModel('sensitive')">API: getModel()</button>
            <button (click)="setFilterValues('sensitive')">API: setFilterValues() - mismatching case </button>
...
public rowData: any[] | null = getData()
    ...
    ((this.gridApi.getToolPanelInstance('filters') as any) as IFiltersToolPanel).expandFilters();
}
...
setFilterValues(type: string) {
    const instance = this.gridApi.getFilterInstance(FILTER_TYPES[type]) as ISetFilter;
