...
const getNextUntitledFileNumber = (existingFiles, folderPath) => {
  const sameFilePath = (f) =>
    same(f.filePath.slice(0, folderPath.length), folderPath);
  const fileInFolder = (f) =>
    f.file && f.filePath.length === folderPath.length + 1;
...
const deleteFiles = (files, pathToRemove) =>
  files
    .slice()
    .filter(
      (f) => !same(f.filePath.slice(0, pathToRemove.length), pathToRemove)
    );

const moveFiles = (exitingFiles, pathToMove, targetPath) => {
  if (same(pathToMove, targetPath)) return exitingFiles;
  return exitingFiles.slice().map((f) =>
    same(f.filePath.slice(0, pathToMove.length), pathToMove)
      ? {
          id: f.id,
          file: f.file,
