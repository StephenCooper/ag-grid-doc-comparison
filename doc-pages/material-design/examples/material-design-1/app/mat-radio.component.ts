...
@Component({
  ...
  styles: [
    `
      ...
      ::ng-deep .mat-radio-container .mat-ripple-element {
        opacity: 0.2;
      }
    `,
  ],
...
export class MatRadioComponent implements ICellEditorAngularComp {
  ...
  private selectFavouriteFruitBasedOnSelectedIndex() {
    ...
    // focus on next tick
    const fruitRadio = this.fruitRadios.find(
      (radio: any) => radio.value === this.favouriteFruit
    );
    window.setTimeout(() => {
  ...
  onKeyDown(event: any): void {
    ...
    if (key === "ArrowUp" || key === "ArrowDown") {
      ...
      if (key == "ArrowUp") {
        ...
        // down
        this.selectedIndex =
          this.selectedIndex === this.fruits.length - 1
            ? 0
            : this.selectedIndex + 1;
      }
      this.selectFavouriteFruitBasedOnSelectedIndex();
